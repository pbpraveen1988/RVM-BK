...
export class CampaignService {
  ...
  async handleCampaignCron() {
    ...
    const queryString = `SELECT c.id,c.utctime,c.timeZone,c.start_date,c.end_date,csv.filename,csv.totalCount,c.intervalMinute,c.lastIndex, a.filename AS audio_filename FROM campaign c INNER JOIN csvfile csv ON c.csvfile_id = csv.id INNER JOIN audio a ON a.id = c.audio_id WHERE c.isCalling = 0 AND c.status = 1`;
